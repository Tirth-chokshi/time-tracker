...
import { Clock, Play, Pause, RotateCcw, CheckCircle, Coffee, Briefcase, Calendar, TrendingUp, Palette, Sun, Moon, Zap, Sparkles, Settings, Activity, Timer, Users, BarChart3, CircleDot, ChevronRight, Target, Flame, Home, BarChart2, Smartphone, Monitor } from 'lucide-react';
...
const WorkHoursTracker = () => {
  ...
  const themes = {
    minimal: {
      name: 'Minimal',
      icon: <CircleDot className="mobile-icon" />,
    ...
    aurora: {
      name: 'Aurora',
      icon: <Sparkles className="mobile-icon" />,
    ...
    dark: {
      name: 'Dark',
      icon: <Moon className="mobile-icon" />,
  ...
  // Create circular progress for mobile
    ...
    <div className={`min-h-screen relative overflow-hidden ${theme.bg}`}>
      ...
      <div className="relative z-10 min-h-screen flex flex-col">
        ...
        <header className={`${theme.surface} ${theme.surfaceHover} backdrop-blur-xl sticky top-0 z-40 border-b border-white/10`}>
          <div className="safe-area-padding px-4 py-3 sm:px-6 sm:py-4">
            <div className="flex items-center justify-between">
              {/* Logo & Title */}
              <div className="flex items-center space-x-3">
                <div className="bg-gradient-primary p-2.5 sm:p-3 rounded-2xl animate-glow shadow-lg">
                  <Timer className="mobile-icon text-white" />
                </div>
                <div>
                  <h1 className={`text-lg sm:text-xl font-bold ${theme.textPrimary} tracking-tight`}>TimeTracker</h1>
                  <p className={`text-xs sm:text-sm ${theme.textMuted} font-medium`}>Work Session Manager</p>
                </div>
              </div>
              
              {/* Time Display with enhanced styling */}
              <div className={`${theme.surface} rounded-2xl px-4 py-3 sm:px-5 sm:py-4 shadow-soft border border-white/20`}>
                <div className={`text-base sm:text-xl font-mono font-bold ${theme.textPrimary} tracking-wider`}>
            ...
            {/* Theme Selector - Enhanced Mobile UI */}
            <div className="flex items-center justify-center mt-4 space-x-2 overflow-x-auto pb-2 scrollbar-hide">
              {Object.entries(themes).map(([key, themeOption]) => (
                <button
                  key={key}
                  onClick={() => setCurrentTheme(key)}
                  className={`flex items-center space-x-2 px-4 py-2 rounded-xl transition-all duration-300 hover:scale-105 active:scale-95 min-w-max ${
                    currentTheme === key
        ...
        <main className="flex-1 p-4 space-y-6 pb-safe">
          ...
          <div className={`${theme.surface} ${theme.surfaceHover} rounded-3xl p-5 sm:p-6 animate-scale-in shadow-medium border border-white/10`}>
            <div className="flex items-center justify-between flex-wrap gap-4">
              <div className="flex items-center space-x-3 flex-wrap gap-2">
                <div className={`flex items-center space-x-2 px-4 py-2.5 sm:px-5 sm:py-3 rounded-full shadow-lg ${
                  currentStatus === 'IN' 
              ...
              <button
                onClick={resetDay}
                className={`${theme.surface} ${theme.textSecondary} ${theme.surfaceHover} px-5 py-2.5 rounded-xl flex items-center space-x-2 hover-lift active:scale-95 border border-white/10 shadow-soft`}
              >
          ...
          {/* Enhanced Clock Section - Mobile Hero */}
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
            {/* Enhanced Clock Card */}
            <div className="lg:col-span-2">
              <div className={`${theme.surface} ${theme.surfaceHover} rounded-3xl p-6 sm:p-8 text-center animate-float shadow-large border border-white/10`}>
                <div className="mb-4">
                ...
                <div className="flex justify-center mb-8">
                  <div className="relative">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className={`w-20 h-20 rounded-full ${currentStatus === 'IN' ? 'bg-gradient-success' : 'bg-gradient-primary'} flex items-center justify-center shadow-lg`}>
                        {currentStatus === 'IN' ? 
                ...
                <button
                  ...
                  } rounded-2xl py-4 text-lg font-bold tracking-wide`}
                >
                  {currentStatus === 'OUT' ? (
                    <>
            ...
            <div className="lg:col-span-3 space-y-6">
              ...
              <div className={`${theme.surface} ${theme.surfaceHover} rounded-3xl p-5 sm:p-6 shadow-large border border-white/10`}>
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center space-x-3">
                    <div className="bg-gradient-primary p-2.5 rounded-xl">
                      <BarChart2 className="mobile-icon text-white" />
                    </div>
                    <div>
                ...
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <div className="text-center p-3 rounded-2xl bg-white/30 backdrop-blur-sm">
                    ...
                    <div className={`text-xs ${theme.textMuted}`}>Work</div>
                  </div>
                  <div className="text-center">
                    <div className={`text-lg sm:text-xl font-bold ${theme.textPrimary}`}>
                      {formatMinutesToHours(dailyStats.totalBreakMinutes)}
                    </div>
                    <div className={`text-xs ${theme.textMuted}`}>Break</div>
                  </div>
                  <div className="text-center">
                    <div className={`text-lg sm:text-xl font-bold ${theme.textPrimary}`}>
                      {getRemainingTime()}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div className={`stat-card ${theme.breakCard} text-white rounded-3xl shadow-large border border-white/10`}>
                  <div className="flex items-center space-x-3">
                    <div className="bg-white/20 p-2.5 rounded-xl">
                      <Coffee className="mobile-icon text-white" />
                    </div>
                    <div>
                ...
                <div className={`stat-card ${theme.remainingCard} text-white rounded-3xl shadow-large border border-white/10`}>
                  <div className="flex items-center space-x-3">
                    <div className="bg-white/20 p-2.5 rounded-xl">
                      <Target className="mobile-icon text-white" />
                    </div>
                    <div>
            ...
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center space-x-3">
                <div className="bg-gradient-primary p-2.5 rounded-xl shadow-lg">
                  <Activity className="mobile-icon text-white" />
                </div>
                <div>
      ...
      {/* Enhanced Break Dialog - Mobile Optimized */}
      {showBreakDialog && (
        <div className="fixed inset-0 bg-black/70 flex items-end sm:items-center justify-center z-50 p-4 backdrop-blur-lg">
          <div className={`${theme.surface} rounded-t-3xl sm:rounded-3xl p-6 sm:p-8 w-full sm:max-w-md animate-scale-in shadow-2xl border border-white/20`}>
            ...
            <div className="flex flex-col space-y-3">
              <button
                ...
                className="w-full bg-gradient-warning text-white font-semibold py-4 px-6 rounded-2xl hover-lift active:scale-95 shadow-lg border border-white/20"
              >
              ...
              <button
                ...
                className="w-full bg-gradient-danger text-white font-semibold py-4 px-6 rounded-2xl hover-lift active:scale-95 shadow-lg border border-white/20"
              >
                <div className="flex items-center justify-center space-x-2">
